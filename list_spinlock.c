...
pthread_spinlock_t* changer_sync;
pthread_spinlock_t* storage_sync;
...
void* _1_thread(void* arg) {
	...
	while(1){
		pthread_spin_lock(storage_sync);
		Node* cur = storage->first;
		pthread_spin_unlock(storage_sync);
		...
		while(cur->next != NULL){
			...
			Node* curLast = cur;
			cur = cur->next;
			pthread_spin_unlock(curLast->sync);
		}
...
void* _change_thread(void* arg) {
	...
	while(1){
		...
		if(rand() % 2){
			...
			pthread_spin_lock(changer_sync);
			++thread_change;
			pthread_spin_unlock(changer_sync);
		}
		pthread_spin_unlock(storage_sync);
		while(1){
			pthread_spin_lock(cur->next->next->sync);
			if(rand() % 2){
...
int main(){
	...
	first->value[0] = 'a';
	first->next = NULL;
	first->sync = malloc(sizeof(pthread_spinlock_t));
	pthread_spin_init(first->sync, PTHREAD_PROCESS_PRIVATE);
	storage->first = first;
	...
	for(int i = 0; i < size - 1; ++i){
		...
		curNew->sync = malloc(sizeof(pthread_spinlock_t));
		pthread_spin_init(curNew->sync, PTHREAD_PROCESS_PRIVATE);
		curNew->next = NULL;
		
		cur->next = curNew;
		cur = cur->next;
	}
	changer_sync = malloc(sizeof(pthread_spinlock_t));
	storage_sync = malloc(sizeof(pthread_spinlock_t));
	pthread_spin_init(changer_sync, PTHREAD_PROCESS_PRIVATE);
	pthread_spin_init(storage_sync, PTHREAD_PROCESS_PRIVATE);
